...
const createError = (message: string, uri: string) => new Error(`[@typescript-utils/typed-webreq - ${new Date().toISOString()}]: An error occured while attempting to fetch data from ${uri}: ${message}`);
...
export default async function webreq(metadata: WebReqMetadataInterface): Promise<WebReqSuccessfulReturnInterface> {
    ...
    if(metadata.api.protocol.startsWith("http") && isNullish(metadata.api.domain)) throw createError(`HTTP / HTTPS requests always require a domain.`, reqURI);
    ...
    try {
        ...
        throw createError("Following error occured while attempting request: "+JSON.stringify(e), reqURI);
    }
    ...
    if(!req.ok) throw createError(`Server${!metadata.api.domain ? "" : " at "+metadata.api.domain} returned HTTP Status "${req.status} - ${mapStatus(req.status.toString())}"`, reqURI);
    ...
    if (contentType?.includes("application/json")) {
        ...
        if (!validation.valid) {
              throw createError(validation.errMessage ?? "Data doesn't match expected return type.", reqURI);
        }
    } else {
        throw createError(`Unsupported Content-Type: ${contentType ?? "missing"}`, reqURI);
    }
